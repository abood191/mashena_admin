...
export default function PoolMap({ snapshot }) {
  const { id } = useParams();
  ...
  const bounds = [];
  if (snapshot?.meetingPoint) bounds.push([snapshot.meetingPoint.lat, snapshot.meetingPoint.lng]);
  ...
  if (snapshot?.routeGeometry && Array.isArray(snapshot.routeGeometry)) {
    routePath = snapshot.routeGeometry.map(p => {
  ...
  // Pool driver assigned if status is MATCHED
  const driverId = snapshot?.driverProfileId ? `shared_ride_driver_${snapshot.sharedRideId}` : `pool_driver_${id}`;
    ...
    <div className="absolute inset-0 z-0">
      <MapContainer center={center} zoom={13} className="w-full h-full z-0" zoomControl={false}>
        ...
        {snapshot?.meetingPoint && (
          <Marker position={[snapshot.meetingPoint.lat, snapshot.meetingPoint.lng]} icon={pickupIcon}>
            <Popup className="text-xs">Meeting Point: {snapshot.meetingPoint.address}</Popup>
          </Marker>
        )}
        ...
        {/* Route (if calculated by backend) */}
        {routePath.length >= 2 && (
          <Polyline positions={routePath} pathOptions={{ color: "#8B5CF6", weight: 5, opacity: 0.8 }} />
        )}

        {/* Pool Members locations (if they are tracked individually) */}
        {snapshot?.members?.map((m, i) => {
          if (m.currentLocation) {
            return (
              <Marker key={`member-${i}`} position={[m.currentLocation.lat, m.currentLocation.lng]} icon={pickupIcon}>
                <Popup className="text-xs">
                  Member: {m.name}<br/>Status: {m.status}
                </Popup>
              </Marker>
            );
          }
