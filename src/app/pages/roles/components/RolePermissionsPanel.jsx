...
export function RolePermissionsPanel({ selectedRole }) {
  ...
  return (
    <div className="rounded-3xl border border-border-subtle bg-surface overflow-hidden flex flex-col min-h-[500px]">
      ...
      <div className="p-5 flex-1 overflow-y-auto space-y-6 relative">
        ...
        {/* Available */}
        <div>
          <div className="text-sm text-foreground font-semibold">{t("roles.available", { defaultValue: "Available Permissions" })}</div>
          <div className="mt-3 space-y-2">
            {available.map((p) => (
              <button
                ...
                disabled={isSaving || isLoading}
                className="w-full flex items-center justify-between rounded-2xl border border-border-subtle bg-foreground/5 hover:bg-foreground/10 px-4 py-3 text-sm text-foreground disabled:opacity-50 transition-colors"
              >
                <span className="truncate">{p.name}</span>
                <span
                  className="h-8 px-3 rounded-xl border border-border-subtle bg-[#4880FF]/10 text-[#4880FF] grid place-items-center text-xs font-semibold"
                >
      ...
      {/* Save Button */}
      <div className="p-5 border-t border-border-subtle bg-foreground/5 shrink-0">
        <button
